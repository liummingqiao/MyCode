...
Page({
  ...
  click:function(e){
    wx.navigateTo({
      url: "/pages/share/share?statusCodeORTaskId=" + this.data.ot.userId,
    })
  },
  /**
   * 生命周期函数--监听页面加载
  ...
  onLoad: function (res) {
    ...
    //获取当前任务的详细信息
wx.request({
  url: 'http://192.168.137.24:8080/v1/open/task/getOneTask/'+res.task_id,
  method:"POST",
  success(e){
    ...
    if (e.data.user_id == that.data.user_id) {
      wx.request({
        url: 'http://192.168.137.24:8080/v1/open/task/getPartnerExamine/' + res.task_id,
        success(e){
      ...
      wx.request({
        url: 'http://192.168.137.24:8080/v1/open/task/pantner?userId='+that.data.user_id+'&taskId='+res.task_id,
        success(e){
          console.log("普通参与者页面");
          console.log(e)
          that.setData({
  ...
  close: function (e) {
    ...
    wx.request({
      url: 'http://192.168.137.24:8080/v1/open/task/toExamine',
      method:"POST",
      data:{
